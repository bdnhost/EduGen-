...
# Paths - Updated for digital literacy course
BASE = Path(__file__).parent
JSON_FILE = (
    BASE / "course_digital_literacy_enhanced.json"
)  # Using enhanced JSON for digital course
MD_FILE = BASE / "course_digital_literacy_masterpiece.md"  # Will create this if needed
OUT_PDF = BASE / "course_digital_literacy_syllabus_enhanced.pdf"
OUT_PPTX = BASE / "course_digital_literacy_master_slides_enhanced.pptx"
LINKS_JSON = BASE / "course_digital_literacy_links_enhanced.json"
MANIFEST_JSON = BASE / "course_digital_literacy_manifest_enhanced.json"

# Local paths only - no web URLs for local use
ASSIGNMENTS_DIR = BASE / "assignments"
DIAGRAMS_DIR = BASE / "diagrams"
SESSION_PDFS_DIR = BASE / "session_pdfs_digital_enhanced"
...
def load_md(path):
    ...
    if not path.exists():
        ...
        # Create a basic markdown file from course data
        course = load_json(JSON_FILE)
        course_data = course.get("course", {})
        md_content = f"""# {course_data.get('name', 'Digital Literacy Course')}

## Course Overview
{course_data.get('description', 'Digital literacy basics for students')}

## Course Details
- **Code:** {course_data.get('code', 'DIGITAL-PRO-2025')}
- **Start Date:** {course_data.get('start_date', '2025-12-15')}
- **Total Sessions:** {course_data.get('total_sessions', 4)}
- **Total Hours:** {course_data.get('total_hours', 20)}
- **Institution:** {course_data.get('institution', 'LearningHub - EduManage')}

## Course Structure
This course covers essential digital literacy skills for students including:
1. Efficient academic search and information filtering
2. File and document organization
3. Assignment submission and digital time management
4. Practical application and project work

## Learning Outcomes
By the end of this course, students will be able to:
- Conduct efficient academic searches using advanced techniques
- Organize digital files and documents systematically
- Manage academic time effectively using digital tools
- Submit assignments professionally in appropriate formats

## Assessment
The course includes:
- Practical exercises for immediate application
- Comprehensive assignments for deeper learning
- Projects for long-term skill development
- Regular quizzes to check understanding

---
*Course automatically generated from digital_literacy_basics_guide.html*
"""
...
def make_syllabus_pdf(course, md_text, out_path):
    ...
    try:
        ...
        # Course title
        course_data = course.get("course", {})
        course_name = course_data.get("name", "Digital Literacy Course")
        ...
        meta_lines = [
            f"Code: {course_data.get('code', 'DIGITAL-PRO-2025')}",
            f"Start: {course_data.get('start_date', '2025-12-15')}",
            f"Sessions: {course_data.get('total_sessions', 4)}",
            f"Hours: {course_data.get('total_hours', 20)}",
            f"Institution: {course_data.get('institution', 'LearningHub - EduManage')}",
        ]
        ...
        for s in sessions:
            title = f"{s.get('session_number', '')} - {s.get('title', '')}"
        ...
        c.save()
        logger.info(f"Enhanced digital syllabus PDF created: {out_path}")
        return str(out_path.relative_to(BASE))
    except Exception as e:
        logger.error(f"Failed to create enhanced digital syllabus PDF: {e}")
        raise
...
def make_session_pdfs(course, out_dir):
    ...
    for s in sessions:
        try:
            sid = s.get("temp_id") or f"s{s.get('session_number', '?')}"
            ...
            # Session title
            title = s.get("title", "Digital Literacy Session")
            ...
            logger.debug(f"Created enhanced digital session PDF: {fname}")
        except Exception as e:
            logger.error(
                f"Failed to create enhanced PDF for digital session {s.get('temp_id', 'unknown')}: {e}"
            )

    logger.info(f"Created {len(sessions)} enhanced digital session PDFs in {out_dir}")
...
def make_pptx(course, out_path):
    ...
    try:
        ...
        course_data = course.get("course", {})
        title.text = course_data.get("name", "Digital Literacy Course")
        subtitle.text = f"{course_data.get('code', 'DIGITAL-PRO-2025')} — Enhanced Digital Literacy Syllabus"
        ...
        slide_layout = prs.slide_layouts[1]
        slide = prs.slides.add_slide(slide_layout)
        slide.shapes.title.text = "Digital Literacy Learning Outcomes"

        body = slide.shapes.placeholders[1].text_frame
        body.text = ""
        outcomes = [
            "Efficient academic search and information filtering",
            "Systematic file and document organization",
            "Effective digital time management",
            "Professional assignment submission",
        ...
        logger.info(f"Enhanced digital PPTX created: {out_path}")
        return str(out_path.relative_to(BASE))
    except Exception as e:
        logger.error(f"Failed to create enhanced digital PPTX: {e}")
        raise
...
def create_links_manifest(course, syllabus_pdf_path, session_links, pptx_path):
    ...
    manifest = {
        "course": {
            ...
            "syllabus_pdf": {
                ...
                "purpose": "Main digital literacy course syllabus document with assignments and diagrams",
                "pedagogical_role": "Overview of course structure, sessions, objectives, and resources",
            },
            "slides": {
                "path": pptx_path,
                "purpose": "Presentation slides for digital literacy instruction",
                "pedagogical_role": "Visual support for lectures and discussions on digital skills",
            },
            ...
            manifest["sessions"].append(
                {
                    ...
                    "session_pdf": {
                        ...
                        "purpose": "Detailed outline for this specific digital literacy session",
                    ...
                    "pedagogical_sequence": f"Session {s.get('session_number', '?')} of {len(sessions)}",
                }
            )
        else:
            manifest["sessions"].append(
    ...
    logger.info(f"Enhanced digital pedagogical manifest created: {MANIFEST_JSON}")
    ...
    logger.info(f"Enhanced digital simple links file created: {LINKS_JSON}")
...
def main():
    """Main function with enhanced pedagogical linking and local resources for digital course."""
    try:
        logger.info("Loading enhanced digital course data...")
        course = load_json(JSON_FILE)
        ...
        logger.info("Generating enhanced digital syllabus PDF...")
        syllabus_path = make_syllabus_pdf(course, md, OUT_PDF)

        logger.info("Generating enhanced digital per-session PDFs...")
        session_links = make_session_pdfs(course, SESSION_PDFS_DIR)

        logger.info("Generating enhanced digital PPTX...")
        pptx_path = make_pptx(course, OUT_PPTX)

        logger.info("Creating enhanced digital pedagogical links manifest...")
        ...
        print("\n" + "=" * 70)
        print("ENHANCED DIGITAL LITERACY COURSE ASSETS - LOCAL USE ONLY")
        print("=" * 70)
        ...
        print(f"\nLocal Resources:")
        print(
            f"  • Assignments: {ASSIGNMENTS_DIR.relative_to(BASE)}/ (3 types x 4 sessions = 12 assignments)"
        )
        print(
            f"  • Diagrams: {DIAGRAMS_DIR.relative_to(BASE)}/ (3 types x 4 sessions = 12 diagrams)"
        )
        ...
        print(f"\nDigital Literacy Focus Areas:")
        print("  • Academic search and information filtering")
        print("  • File and document organization")
        print("  • Digital time management")
        print("  • Assignment submission and presentation")

        print("\n" + "=" * 70)
        print("All enhanced digital literacy assets generated successfully!")
        print("=" * 70)

        logger.info(
            "All enhanced digital assets and pedagogical links generated successfully!"
        )

    except Exception as e:
        logger.error(f"Enhanced digital script failed: {e}")
        sys.exit(1)
