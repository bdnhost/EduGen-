...
export class OpenRouterProvider {
   ...
   */
  async generateGuide(request, modelId = 'deepseek/deepseek-chat') {
    // Handle both old and new API formats
    const topic = typeof request === 'string' ? request : request.topic;
    const userDescription = typeof request === 'string' ? arguments[1] : request.description;
    
    const prompt = this.buildPrompt(request, topic, userDescription);
  ...
  buildPrompt(request, topic, userDescription) {
    // Extract matireal-compatible fields
    const category = request.category;
    const emoji = request.emoji;
    const shortDescription = request.shortDescription;
    const teasers = request.teasers;
...
`;
    ...
    if (category) {
      prompt += `\n**REQUIRED Category:** Use "${category}" as the category.`;
    }
    if (emoji) {
      prompt += `\n**REQUIRED Emoji:** Use "${emoji}" as the emoji.`;
    }
    if (shortDescription) {
      prompt += `\n**REQUIRED Short Description:** Use "${shortDescription}" as the shortDescription.`;
    }
    if (teasers && Object.values(teasers).some(t => t.trim())) {
      prompt += `\n**REQUIRED Teasers:** Use these specific teasers:`;
      if (teasers.idea) prompt += `\n- teaserIdea: "${teasers.idea}"`;
      if (teasers.warning) prompt += `\n- teaserWarning: "${teasers.warning}"`;
      if (teasers.secret) prompt += `\n- teaserSecret: "${teasers.secret}"`;
      if (teasers.bonus) prompt += `\n- teaserBonus: "${teasers.bonus}"`;
    }
    if (assignments && (assignments.basic?.length > 0 || assignments.advanced?.length > 0)) {
      prompt += `\n**REQUIRED Assignments:** Incorporate these assignments into the content:`;
      if (assignments.basic?.length > 0) {
        prompt += `\n- Basic tasks: ${assignments.basic.join(', ')}`;
      }
      if (assignments.advanced?.length > 0) {
        prompt += `\n- Advanced tasks: ${assignments.advanced.join(', ')}`;
      }
    }
    if (faq && faq.length > 0) {
      prompt += `\n**REQUIRED FAQ:** Include these FAQ items:`;
      faq.forEach((item, i) => {
        if (item.question && item.answer) {
          prompt += `\n- Q${i+1}: ${item.question} A: ${item.answer}`;
        }
      });
    }
